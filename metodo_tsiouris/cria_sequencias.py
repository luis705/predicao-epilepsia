...
Uso no anaconda:
    ...
    "PreIctal" no nome.
    n_de_vetores é o número de vetores por sequência, o Pires usou 30
...
for i in range(len(treino_preictal) // 30):
    np.savetxt(
        f"seqTestePreictal{i}.txt", treino_preictal[treino_preictal.shape[0] - 30 :]
    )
    treino_preictal = treino_preictal[0 : treino_preictal.shape[0] - 30]

for i in range(len(teste_interictal) // 30):
    np.savetxt(
        f"seqTesteInterictal{i}.txt",
        teste_interictal[teste_interictal.shape[0] - 30 :],
    )
    teste_interictal = teste_interictal[0 : treino_interictal.shape[0] - 30]

for i in range(len(teste_preictal) // 30):
    np.savetxt(
        f"seqTestePreictal{i}.txt", teste_preictal[teste_preictal.shape[0] - 30 :]
    )
    teste_preictal = teste_preictal[0 : teste_preictal.shape[0] - 30]
