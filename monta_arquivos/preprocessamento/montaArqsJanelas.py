...
class Packing:
    def __init__(self, objeto_Tk):
        ...
        objeto_Tk.title(
            'Monta arquivos com janelas de amostras de EEG - versão 20/03/2023'
        )
        ...
        self.lbNomeArqSumario = Label(
            self.frNomeArqSumario,
            textvariable=self.txtVarNomeArqSumario,
            fg='darkblue',
            height=3,
        ).pack(side=LEFT)
        self.entryNomeArqSumario = Entry(self.frNomeArqSumario, width=80)
        self.entryNomeArqSumario.insert(
            END, '/home/rpires/chb-mit-scalp-eeg-database-1.0.0/'
        )
        self.entryNomeArqSumario.pack(side=LEFT)
        self.btNomeArqSumario = Button(
            self.frNomeArqSumario,
            text='procura',
        ...
        self.txtVarDuracaoJanela.set(
            'duração de cada janela de EEG (segundos):'
        )
        self.lbDuracaoJanela = Label(
            self.frParams,
        ...
        self.entryLocArqEEG.insert(
            END, '/home/rpires/chb-mit-scalp-eeg-database-1.0.0'
        )
        self.entryLocArqEEG.pack(side=LEFT)
        self.btLocArqEEG = Button(
            self.frLocArqEEG, text='procura', command=self.prssBtLocArqEEG
        )
        self.btLocArqEEG.pack(side=LEFT)

        self.frLocJanEEG = Frame(objeto_Tk)
        self.frLocJanEEG.pack()
        self.lbLocJanEEG = Label(
            self.frLocJanEEG,
        ...
        self.btLocJanEEG = Button(
            self.frLocJanEEG, text='procura', command=self.prssBtLocJanEEG
        )
        self.btLocJanEEG.pack(side=LEFT)

        self.frLerSumario = Frame(objeto_Tk)
        self.frLerSumario.pack()
        self.btLerSumario = Button(
            self.frLerSumario,
            text='ler sumário',
        ...
        self.lbNumCrisesDoPaciente = Label(
            self.frNumCrisesDoPaciente,
        ...
        self.lbCrisesTreino = Label(
            self.frLtCrisesTreino,
            textvariable=self.txtVarCrisesTreino,
            fg='darkblue',
            height=4,
        )
        self.lBxLtCrisesTreino = Listbox(
            self.frLtCrisesTreino, selectmode=MULTIPLE, state=DISABLED
        )

        self.btEscolherCrisesTreino = Button(
            self.frLtCrisesTreino,
        ...
        self.lbCrisesTeste = Label(
            self.frLtCrisesTeste,
            textvariable=self.txtVarCrisesTeste,
            fg='red',
            height=4,
        )
        self.lBxLtCrisesTeste = Listbox(
            self.frLtCrisesTeste, selectmode=MULTIPLE, state=DISABLED
        )

        self.btEscolherCrisesTeste = Button(
            self.frLtCrisesTeste,
            text='escolher crises',
            state=DISABLED,
        ...
        self.btEscolhaCrisesConcluida = Button(
            self.frBtEscolhaCrisesConcluida,
        ...
        self.lbEscolherPropTreino = Label(self.frLtCrisesTreino, text=strAux)
        self.intVarEscolherPropTreino = IntVar()
        self.scEscolherPropTreino = Scale(
            self.frLtCrisesTreino,
            orient=HORIZONTAL,
        ...
        self.lbEscolherPropTeste = Label(self.frLtCrisesTeste, text=strAux)
        self.intVarEscolherPropTeste = IntVar()
        self.scEscolherPropTeste = Scale(
            self.frLtCrisesTeste,
            orient=HORIZONTAL,
            variable=self.intVarEscolherPropTeste,
        )

        self.btEscolhaPropTreinoTesteConcluida = Button(
            self.frLtsCrises,
    ...
    def prssBtNomeArqSumario(self):
        self.strNomeArqSumario = filedialog.askopenfilename(
    ...
    def prssBtEscolherCrisesTreino(self):
        ...
        for indice in selecionados:
            numUmaCriseTreino = self.dadosSumario.ltCrisesParaExperimentos[
                indice
            ]
            self.ltCrisesParaTreino.append(numUmaCriseTreino)
        self.txtVarNumCrises.set('')
        self.txtVarCrisesTreino.set(
            'crises p/ treino: ' + str(self.ltCrisesParaTreino)
        )
        ...
        for itInicPeriodo in self.dadosSumario.ltInicPeriodo:
            if itInicPeriodo.antesDeQualCrise in self.ltCrisesParaTreino:
                if itInicPeriodo.nomePeriodo == 'interictal':
                    ...
                    itInicPeriodo.nomePeriodo == 'pré-ictal' and esperaPreIctal
                ):
                    print(itInicPeriodo)
                    tIniPreIctal = itInicPeriodo.tIni
                    print('tempo fim:   ', tIniPreIctal, 's')
                    intrvAux = leSummary.IntervaloSeg(
                        tIniInterictal, tIniPreIctal
                    )
                    self.ltInterictaisPTreino.append(
                        (intrvAux, itInicPeriodo.antesDeQualCrise)
                    )
                    esperaPreIctal = False
        ...
        print(
            'tempo total de interictal para treino:',
            self.tTotalInterictalTreino,
            's',
        )
        print('lista de interictais para treino:')
        print(*self.ltInterictaisPTreino, sep='\n')
        self.numJanTotalInterictalTreino = int(
            self.tTotalInterictalTreino / self.duracJanelaSegundos
        )
        strMensagem = (
            '\nt. total interictal p/ treino: '
            + str(self.tTotalInterictalTreino)
            + ' s ('
            + str(self.numJanTotalInterictalTreino)
            + ' janelas)'
        )
        ...
        for itInicPeriodo in self.dadosSumario.ltInicPeriodo:
            if itInicPeriodo.antesDeQualCrise in self.ltCrisesParaTreino:
                if (
                    itInicPeriodo.nomePeriodo == 'pré-ictal'
                    and not esperaPreIctal
                ):
                    print(itInicPeriodo)
                    tIniPreIctal = itInicPeriodo.tIni
                    ...
                    self.ltPreIctaisPTreino.append(
                        (intrvAux, itInicPeriodo.antesDeQualCrise)
                    )
                    ...
                    esperaPreIctal = False
        print(
            'tempo total de pré-ictal para treino:',
            self.tTotalPreIctalTreino,
            's',
        )
        print('lista de pré-ictais para treino:')
        print(*self.ltPreIctaisPTreino, sep='\n')
        self.numJanTotalPreIctalTreino = int(
            self.tTotalPreIctalTreino / self.duracJanelaSegundos
        )
        strMensagem = (
            '\nt. total pré-ictal p/ treino: '
            + str(self.tTotalPreIctalTreino)
            + ' s ('
            + str(self.numJanTotalPreIctalTreino)
            + ' janelas)'
        )
        strAux = self.txtVarCrisesTreino.get()
        strAux += strMensagem
    ...
    def prssBtEscolherCrisesTeste(self):
        ...
        for indice in selecionados:
            numUmaCriseTeste = self.dadosSumario.ltCrisesParaExperimentos[
                indice
            ]
            self.ltCrisesParaTeste.append(numUmaCriseTeste)
        self.txtVarNumCrises.set('')
        self.txtVarCrisesTeste.set(
            'crises p/ teste: ' + str(self.ltCrisesParaTeste)
        )
        ...
        for itInicPeriodo in self.dadosSumario.ltInicPeriodo:
            if itInicPeriodo.antesDeQualCrise in self.ltCrisesParaTeste:
                if itInicPeriodo.nomePeriodo == 'interictal':
                    ...
                    self.ltInterictaisPTeste.append(
                        (intrvAux, itInicPeriodo.antesDeQualCrise)
                    )
                    esperaPreIctal = False
                    duracInterictal = tIniPreIctal - tIniInterictal
        ...
        print(
            'tempo total de interictal para teste:',
            self.tTotalInterictalTeste,
            's',
        )
        print('lista de interictais para teste:')
        print(*self.ltInterictaisPTeste, sep='\n')
        self.numJanTotalInterictalTeste = int(
            self.tTotalInterictalTeste / self.duracJanelaSegundos
        )
        strMensagem = (
            '\nt. total interictal p/ teste: '
            + str(self.tTotalInterictalTeste)
            + ' s ('
            + str(self.numJanTotalInterictalTeste)
            + ' janelas)'
        )
        ...
        for itInicPeriodo in self.dadosSumario.ltInicPeriodo:
            if itInicPeriodo.antesDeQualCrise in self.ltCrisesParaTeste:
                if (
                    ...
                    esperaPreIctal = False
        print(
            'tempo total de pré-ictal para teste:',
            self.tTotalPreIctalTeste,
            's',
        )
        print('lista de pré-ictais para teste:')
        print(*self.ltPreIctaisPTeste, sep='\n')
        self.numJanTotalPreIctalTeste = int(
            self.tTotalPreIctalTeste / self.duracJanelaSegundos
        )
        strMensagem = (
            '\nt. total pré-ictal p/ teste: '
            + str(self.tTotalPreIctalTeste)
            + ' s ('
            + str(self.numJanTotalPreIctalTeste)
            + ' janelas)'
        )
        strAux = self.txtVarCrisesTeste.get()
        strAux += strMensagem
    ...
    def prssBtLerSumario(self):
        ...
        self.numAmostrasPorJanela = (
            self.duracJanelaSegundos * self.numAmostrasPorSegundo
        )
        self.caminhoPArqsEEGcompactados = self.entryLocArqEEG.get()
        self.caminhoPArqsJanelasEEG = self.entryLocJanEEG.get()
        self.dadosSumario = leSummary.DadosSumario(
            nomeArqSummaryDoPaciente, duracMinutPreIctal, duracMinutPosIctal
        )
        self.txtVarNumCrises.set(
            'Este paciente teve '
        ...
        print(
            '\nself.caminhoPArqsEEGcompactados =',
            self.caminhoPArqsEEGcompactados,
        )
        arqsNoCaminho = [
            arq
            for arq in listdir(self.caminhoPArqsEEGcompactados)
    ...
    def prssBtEscolhaPropTreinoTesteConcluida(self):
        ...
        self.numJanInterictalTreino = int(
            self.numJanTotalInterictalTreino * self.propTreino / 100
        )
        print(
            'número de janelas interictais para treino:',
            self.numJanInterictalTreino,
        )
        self.numJanPreIctalTreino = int(
            self.numJanTotalPreIctalTreino * self.propTreino / 100
        )
        print(
            'número de janelas pré-ictais para treino:',
            self.numJanPreIctalTreino,
        )
        strMensagem = (
            '\nnúmero de janelas p/ treino: '
            + str(self.numJanInterictalTreino)
            + ' interictais e '
            + str(self.numJanPreIctalTreino)
            + ' pré-ictais.'
        )
        strAux = self.txtVarCrisesTreino.get()
        strAux += strMensagem
        ...
        self.numJanInterictalTeste = int(
            self.numJanTotalInterictalTeste * self.propTeste / 100
        )
        print(
            'número de janelas interictais para teste:',
            self.numJanInterictalTeste,
        )
        self.numJanPreIctalTeste = int(
            self.numJanTotalPreIctalTeste * self.propTeste / 100
        )
        print(
            'número de janelas pré-ictais para teste:',
            self.numJanPreIctalTeste,
        )
        strMensagem = (
            '\nnúmero de janelas p/ teste: '
            + str(self.numJanInterictalTeste)
            + ' interictais e '
            + str(self.numJanPreIctalTeste)
            + ' pré-ictais.'
        )
        strAux = self.txtVarCrisesTeste.get()
        strAux += strMensagem
        self.txtVarCrisesTeste.set(strAux)
        self.intervaloInicJanInterictalTreino = (
            self.tTotalInterictalTreino / self.numJanInterictalTreino
        )
        print(
        ...
        self.intervaloInicJanPreIctalTreino = (
            self.tTotalPreIctalTreino / self.numJanPreIctalTreino
        )
        print(
            'intervalo entre inícios de janelas pré-ictais treino:',
            self.intervaloInicJanPreIctalTreino,
            's',
        )
        self.intervaloInicJanInterictalTeste = (
            self.tTotalInterictalTeste / self.numJanInterictalTeste
        )
        print(
            'intervalo entre inícios de janelas interictais teste:',
            self.intervaloInicJanInterictalTeste,
            's',
        )
        self.intervaloInicJanPreIctalTeste = (
            self.tTotalPreIctalTeste / self.numJanPreIctalTeste
        )
        print(
            'intervalo entre inícios de janelas pré-ictais teste:',
            self.intervaloInicJanPreIctalTeste,
            's',
        )
        print('\ninícios de janelas para treino e teste:\n')
        numAlgarismosArqInterictalTreino = math.ceil(
            math.log10(self.numJanInterictalTreino)
        )
        numAlgarismosArqPreIctalTreino = math.ceil(
            math.log10(self.numJanPreIctalTreino)
        )
        ...
        for it in self.ltInterictaisPTreino:
            ...
            while inicJan < limite:
                nomeArqEDF, tNoArq = self.dadosSumario.ltTAcumArqEdf.emQualArq(
                    inicJan
                )
                if nomeArqEDF.find('fora') < 0:
                    if nomeArqEDF != nomeArqEDFanterior:
                        umEEG = eegUmArqPhysionet.EEGumArqPhysionet(
                            self.caminhoPArqsEEGcompactados
                            + '/'
                            + self.dicEdfAscii[nomeArqEDF]
                        )
                        nomeArqEDFanterior = nomeArqEDF
                    self.ltTInicJanIntericTreino.append(
                        (self.dicEdfAscii[nomeArqEDF], int(tNoArq))
                    )
                    nomeArqJan = (
                        self.caminhoPArqsJanelasEEG
            ...
            while inicJan < limite:
                ...
                if nomeArqEDF.find('fora') < 0:
                    if nomeArqEDF != nomeArqEDFanterior:
                        ...
                        nomeArqEDFanterior = nomeArqEDF
                    self.ltTInicJanPreIcTreino.append(
                        (self.dicEdfAscii[nomeArqEDF], int(tNoArq))
                    )
                    nomeArqJan = (
                        self.caminhoPArqsJanelasEEG
        ...
        numAlgarismosArqInterictalTeste = math.ceil(
            math.log10(self.numJanInterictalTeste)
        )
        numAlgarismosArqPreIctalTeste = math.ceil(
            math.log10(self.numJanPreIctalTeste)
        )
            ...
            while inicJan < limite:
                ...
                if nomeArqEDF.find('fora') < 0:
                    if nomeArqEDF != nomeArqEDFanterior:
                        ...
                        nomeArqEDFanterior = nomeArqEDF
                    self.ltTInicJanIntericTeste.append(
                        (self.dicEdfAscii[nomeArqEDF], int(tNoArq))
                    )
                    nomeArqJan = (
                        self.caminhoPArqsJanelasEEG
            ...
            while inicJan < limite:
                ...
                if nomeArqEDF.find('fora') < 0:
                    if nomeArqEDF != nomeArqEDFanterior:
                        ...
                        nomeArqEDFanterior = nomeArqEDF
                    self.ltTInicJanPreIcTeste.append(
                        (self.dicEdfAscii[nomeArqEDF], int(tNoArq))
                    )
                    nomeArqJan = (
                        self.caminhoPArqsJanelasEEG
