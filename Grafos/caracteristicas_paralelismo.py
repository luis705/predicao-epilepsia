...
def salva_caracteristicas(paciente, caminho, arquivo):
    ...
    for intervalo in tqdm(range(sinais.shape[0] // 1280), desc=paciente):
...
if __name__ == "__main__":
    ...
    for paciente in tqdm(pacientes, desc="Pacientes"):
        ...
        caminho = os.path.join("data", paciente)
        arquivos = os.listdir(caminho)
        falta = [arquivo for arquivo in arquivos if arquivo not in terminados]
        with concurrent.futures.ProcessPoolExecutor() as executor:
            [executor.submit(salva_caracteristicas, paciente, caminho, arquivo) for arquivo in falta]
