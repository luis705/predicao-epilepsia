...
class Horario:
    ...
    def emSegundos(self):
        return self.h * 3600 + self.min * 60 + self.s
...
class IntervaloSeg:
    ...
    def __repr__(self):
        return "de " + repr(self.tIni) + " s a " + repr(self.tFin) + " s"
...
class DadosUmArqEdf:
    ...
    def __repr__(self):
        return (
            ...
            + repr(self.dia)
            + ", de "
            + repr(self.horarioIni)
            + " a "
            + repr(self.horarioFin)
            + ", duração: "
            + repr(self.duracaoArq)
            + " s, ictais: "
            + str(self.ltPeriodoIctal)
            + "\n"
        )
...
class InicPeriodo:
    ...
    def __repr__(self):
        return (
            "t.acum.início: "
            + repr(self.tIni).rjust(8)
            + " s, t.no arq: "
            + repr(self.tNoArq).rjust(6)
            + " s, "
            + repr(self.nomePeriodo).ljust(20)
            + repr(self.nomeArq).ljust(20)
            + ", antes da crise "
            + str(self.antesDeQualCrise)
        )
...
class TAcumArqEdf:
    ...
    def __repr__(self):
        return (
            ...
            + self.nomeArq
            + " de "
            + repr(self.tAcumIni).rjust(8)
            + " s a "
            + repr(self.tAcumFin).rjust(8)
            + " s"
        )
...
class DadosSumario:
    def __init__(
        self, nomeArqSummaryDoPaciente, duracMinutPreIctal, duracMinutPosIctal
    ):
        ...
        self.ltCrisesParaExperimentos = (
            []
        )  # Compatíveis com durações do pré-ictal e do pós-ictal.
        with open(nomeArqSummaryDoPaciente) as arqSummary:
            ...
            while linhaSum:
                if linhaSum.find("Name") > -1:
                    ...
                    dadosUmArqEdf.duracaoArq = (
                        dadosUmArqEdf.horarioFin - dadosUmArqEdf.horarioIni
                    )
    ...
    def preencheLtInicPeriodo(self):
        ...
        for itDadosUmArqEdf in self.ltDadosArqEdf:
            ...
            if primeiro == False:
                ...
                if intervalo < 0:
                    intervalo += 24 * 60 * 60
                tmp = InicPeriodo(
                    tAcumulado, "n.a.", "intervalo", "fora de arquivo"
                )
                self.ltInicPeriodo.append(tmp)
                tAcumulado += intervalo
            tmp = InicPeriodo(
                tAcumulado, 0, "início de arquivo", itDadosUmArqEdf.nomeArqEdf
            )
            self.ltInicPeriodo.append(tmp)
            tmp = TAcumArqEdf(
                itDadosUmArqEdf.nomeArqEdf,
            ...
            for itPeriodoIctal in itDadosUmArqEdf.ltPeriodoIctal:
                ...
                self.ltInicPeriodo.append(tmp)
                tmp = InicPeriodo(
                    tAcumulado + itPeriodoIctal.tIni,
                    -1,
                    "ictal",
                    itDadosUmArqEdf.nomeArqEdf,
                )
                self.ltInicPeriodo.append(tmp)
                tmp = InicPeriodo(
                    tAcumulado + itPeriodoIctal.tFin,
                    -1,
                    "pós-ictal",
                    itDadosUmArqEdf.nomeArqEdf,
                )
        ...
        for itInicPeriodo in self.ltInicPeriodo:
            if itInicPeriodo.nomeArq == "pendente":
                itInicPeriodo.nomeArq, tNoArq = self.ltTAcumArqEdf.emQualArq(
                    itInicPeriodo.tIni
                )
        ...
        for itInicPeriodo in self.ltInicPeriodo:
            if itInicPeriodo.nomePeriodo == "ictal":
                ...
                if itInicPeriodo.antesDeQualCrise > 1:
                    ...
                    print(
                        "intervalo desde o fim da última crise:",
                        intervalo,
                        "s =",
                        int(intervalo / 60),
                        "min",
                    ...
                    if intervalo > self.duracSegundosPreIctal:
                        print(
                            "Está suficientemente distante da crise anterior."
                        )
                        self.ltCrisesParaExperimentos.append(
                            itInicPeriodo.antesDeQualCrise
                        )
                    else:
                        print(
                            "Não está suficientemente distante da crise anterior."
                        )
                else:  # É a primeira crise:
                    if itInicPeriodo.tIni > self.duracSegundosPosIctal:
                        print(
                            "Está suficientemente distante do início geral do tempo."
                        )
                        self.ltCrisesParaExperimentos.append(
                            itInicPeriodo.antesDeQualCrise
                        )
